...
(function(b$, gadgets, bd, be, bc, window, $) {
    ...
    var properties = {
        showPanelOnEvent: {
            name: 'showPanelOnEvent',
            label: 'Show Panel On Event'
        },
        onPanelLoadedEvent: {
            name: 'onPanelLoadedEvent',
            label: 'On Panel Loaded Event'
        },
        onPanelHiddenEvent: {
            name: 'onPanelHiddenEvent',
            label: 'On Panel Hidden Event'
        }
    };
    ...
    var DeckContainer = Container.extend(function(bdomDocument, node) {
        ...
        PANEL: 'container-panel', //still needs nice way to get the correct panel from the catalog
        ...
        savingPanels: 0, //number sof panels asynchronously being saved
        ...
        DOMReady: function(e) {
            ...
            window.setTimeout(function() {
                ...
                if (showControls) {
                    ...
                    if (bd.designMode) {
                        rs.addURI('text/javascript', bd.contextRoot + '/static/containers/%5BBBHOST%5D/container-deck/scripts/deck-designmode.js');
            ...
            var showPanelEventNames = [];

            for (var i = 0; i < this.model.childNodes.length; i++) {
                var panel = this.model.childNodes[i],
                    panelName = panel.name,
                    showPanelEventName = panel.getPreference(properties.showPanelOnEvent.name);

                if (showPanelEventName && showPanelEventNames.indexOf(showPanelEventName) === -1) {
                    showPanelEventNames.push(showPanelEventName);
                    gadgets.pubsub.subscribe(showPanelEventName, self.showPanel.bind(self, panelName));
                }
            }
        ...
        _displayInitialPanel: function() {
            ...
            // Using state2url to show a panel in a deck container is a very good thing but there is something missing
            // that we pretend to fix with the following snippet of code.
            // The problem is that if the deck container is inside a lightbox it will never be showed up.
            // The following snippet of code checks if the deck container is child of a lighbox and then it shows it
            // before showing the panel.
            if (this.state && this.getPanel(this.state)) {
                var parent = this.parentNode;
                var lightboxEvents = [];
                //!!RECURSIVE!! Traversing the parents until it finds a Manageable Area.
                while (parent && parent.model && parent.model.nodeName && parent.model.extendedItemName !== "Manageable_Area_BlankPage") {
                    if (parent.model.extendedItemName === 'container-lightbox') {
                        lightboxEvents.unshift(parent.model.getPreference('open-trigger'));
                    }
                    parent = parent.parentNode;
                }
                // If the user has nested one or more lightbox inside a lightbox it will manage to open them in the correct order.
                lightboxEvents.forEach(function (event) {
                    gadgets.pubsub.publish(event);
                });
            }
         ...
         *  @param id: (string) name or index of the panel
         */
        getPanel: function(id) {
            var panels = this._getPanels() || [];
            var subsetPanels = [];
            var panel = null;
            // Based on id when calling it by position.
            if(typeof id === 'number' && id < panels.length) {
                panel = panels[id];
            } else if(typeof id === 'string') {
                // Based on id when calling it by uuid.
                subsetPanels = panels.filter(function(_panel) {
                    var result = false;
                    if(_panel.model && _panel.model.nodeName === 'Container') {
                        result = _panel.model.name === id;
                    }
                    return result;
                });
                if(subsetPanels.length) {
                    panel = subsetPanels[0];
                }
                // Based on id when calling it by Panel title
                if(!panel) {
                    subsetPanels = panels.filter(function(_panel) {
                        var result = false;
                        if(typeof _panel.getPreference === 'function') {
                            result = _panel.getPreference('title').toLowerCase() === unescape(id).toLowerCase();
                        }
                        return result;
                    });
                    if(subsetPanels.length) {
         ...
         *  @param panelId: (string) name or index of the panel
         */
        showPanel: function(panelId) {
            ...
            if (panel) {
                ...
                var currentActivePanel = panels[this.panelsActive[this.model.name] || 0];
                if (currentActivePanel) {
                    $(this.htmlAreas).hide().removeClass('active');
                    gadgets.pubsub.publish(
                        currentActivePanel.getPreference(properties.onPanelHiddenEvent.name),
                        currentActivePanel.model.name
                    );
                }
                ...
                gadgets.pubsub.publish('DeckPanelLoaded', panel);
                gadgets.pubsub.publish(this.model.name + '-DeckPanelLoaded', panel.model.name);

                var onPanelLoadedEventName = panel.getPreference(properties.onPanelLoadedEvent.name);
                if (onPanelLoadedEventName) {
                    gadgets.pubsub.publish(onPanelLoadedEventName, panel.model.name);
                }
            } else {
                return false;
            }
         ...
         * @param {Object} [child] Optional. the child object to load. If undefined, all children are loaded
         * @param {Function} [callback] called when the children have been appended to the DOM and model is initialized.
         */
        _loadChildren: function(child, callback) {
            var self = this;
            child.loadChildren(undefined, function(item) {
                self._enableTools(item, self);
                if (callback) callback.call(self, item);
            });
        },

        // enable design tools recursively
        _enableTools: function (item, self) {
            if (bd && bd.designMode && item && item.showDesignTools) {
                item.showDesignTools();
                $.each(item.childNodes || [], function(){
                    self._enableTools(this, self);
                });
            }
        },
        ...
        addPanel: function(data, callback) {
            ...
            var defaults = {
                ...
                properties: [{
                    ...
                    name: properties.showPanelOnEvent.name,
                    label: properties.showPanelOnEvent.label,
                    type: 'string',
                    value: '',
                    viewHint: 'input-text,designModeOnly,manager'
                }, {
                    name: properties.onPanelLoadedEvent.name,
                    label: properties.onPanelLoadedEvent.label,
                    type: 'string',
                    value: '',
                    viewHint: 'input-text,designModeOnly,manager'
                }, {
                    name: properties.onPanelHiddenEvent.name,
                    label: properties.onPanelHiddenEvent.label,
                    type: 'string',
                    value: '',
                    viewHint: 'input-text,designModeOnly,manager'
                }]
            ...
            var createPanelInstanceTemplate = be.utils.processXMLTemplateByUrl(b$.portal.config.serverRoot + '/static/containers/[BBHOST]/container-deck/templates/xml/createPanel.xml', tplData);
            ...
            // load model of the panel container
         ...
         */
        loadByBehavior: function(childBehaviorTag, callback, preventRepeat) {
            var matchingChildren = this._findMatchingChildrenByTag(this, childBehaviorTag);
            if (matchingChildren.length > 0) {
                if(preventRepeat === true) {
                    this.setPreference('state', matchingChildren[0].model.name);
                ...
                if(preventRepeat === true) {
                    ...
                    window.setTimeout(this.loadByBehavior.bind(this, childBehaviorTag, callback, true));
                }
            }
        },
        /**
