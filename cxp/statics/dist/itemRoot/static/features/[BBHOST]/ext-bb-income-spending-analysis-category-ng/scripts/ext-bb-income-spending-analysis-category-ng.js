...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _vendorBbUibButtons = __webpack_require__(3);
	...
	var _uiBbChartjsChartDonutNg = __webpack_require__(7);
	
	var _uiBbChartjsChartDonutNg2 = _interopRequireDefault(_uiBbChartjsChartDonutNg);
	
	var _uiBbChartjsChartBarNg = __webpack_require__(8);
	...
	var _uiBbChangePeriodNg = __webpack_require__(10);
	
	var _uiBbChangePeriodNg2 = _interopRequireDefault(_uiBbChangePeriodNg);
	
	var _vendorBbUibAccordion = __webpack_require__(11);
	...
	var dependencyKeys = exports.dependencyKeys = [_vendorBbAngularNgAria2.default, _vendorBbUibButtons2.default, _uiBbI18nNg2.default, _uiBbSubstituteErrorNg2.default, _uiBbFormatAmountNg2.default, _uiBbChartjsChartDonutNg2.default, _uiBbChartjsChartBarNg2.default, _uiBbEmptyStateNg2.default, _uiBbChangePeriodNg2.default, _vendorBbUibAccordion2.default];
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _libBbStyles = __webpack_require__(13);
	 ...
	 * processed items
	 */
	var processAnalysisCategoryItems = function processAnalysisCategoryItems(data) {
	...
	var processAnalysisCategorySeries = function processAnalysisCategorySeries(series, data) {
	  ...
	  series.labels.forEach(function (categoryName, index) {
	    ...
	    colors[index] = (0, _libBbStyles2.default)('.bb-transaction-category-' + categoryClass, 'color', true, false);
	  });
	...
	var processTurnoverSeries = function processTurnoverSeries(series, data) {
	  series.datasets.forEach(function (dataset, index) {
	    var datasetColor = (0, _libBbStyles2.default)('.chart-bar-dataset-' + (dataset.dataKey ? _constants.DATA_KEYS[dataset.dataKey] : index), 'color');
...
/***/ (function(module, exports) {
	 ...
	 * @type {object}
	 */
	var DATA_KEYS = exports.DATA_KEYS = {
	  debitAmount: 0,
	  creditAmount: 1
	};
	 ...
	 * @type {string}
	 */
	var DEFAULT_WIDGET_SELECTOR = exports.DEFAULT_WIDGET_SELECTOR = '.ext-bb-income-spending-analysis-category-ng';
	 ...
	 * @type {string}
	 */
	var PARAM_ERROR = exports.PARAM_ERROR = 'error.load.params';
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var calculatePosition = function calculatePosition(portion, center, radius) {
	  ...
	  var position = {
	    left: Math.round(Math.cos(angle) * chartRadius + center + _constants.CHART_SECTOR_SHIFT_SIZE),
	    top: Math.round(-Math.sin(angle) * chartRadius + center + _constants.CHART_SECTOR_SHIFT_SIZE)
	  };
	 ...
	 */
	var setTooltipPosition = function setTooltipPosition(element, tooltip, chart, item, isNarrow) {
	  ...
	  var xOffset = void 0;
	  if (isNarrow) {
	    xOffset = circlePosition < 50 ? element.clientWidth + arrowOutSize + additionalOffset / 3 : -arrowOutSize - additionalOffset / 3;
	  } else {
	    xOffset = circlePosition > 50 ? element.clientWidth + arrowOutSize + additionalOffset / 3 : -arrowOutSize - additionalOffset / 3;
	  }
	  ...
	  Object.assign(element.style, calculatePosition(circlePosition, chart.outerRadius, chart.outerRadius + additionalOffset, Math.round(xOffset), Math.round(yOffset)));
	};
	...
	var populateSelectedSectorObject = function populateSelectedSectorObject($ctrl, activeSectorName) {
	  return {
	    ...
	    arrangementsIds: $ctrl.selectedProducts.map(function (product) {
	      return product.id;
	    })
	  };
	};
	...
	var helpers = function helpers(context) {
	  ...
	  var getNewSliceLabel = function getNewSliceLabel(category, amount, topHalf) {
	    ...
	    if (isParentSelected) {
	      ...
	      categoryIdentifier.className = 'fa chart-donut-slice-icon bb-transaction-category-' + categoryToIconClass(category);
	    }
	  ...
	  var renderSliceLabels = function renderSliceLabels(chartInstance) {
	    ...
	    chartInstance.config.data.analysisItems.forEach(function (item) {
	      ...
	      var circlePosition = Math.round(item.totalPortion - item.portion / 2);
	      ...
	      setTimeout(function () {
	        ...
	        var yOffset = Math.round(labelOffset + element.clientHeight / 2 + (topHalf ? element.clientHeight / 6 : -element.clientHeight / 3));
	
	        var radius = chartInstance.outerRadius + labelOffset;
	        Object.assign(element.style, calculatePosition(circlePosition, radius, radius, Math.round(labelOffset + element.clientWidth / 2), yOffset), { opacity: 1 });
	      }, 0);
	    });
	  };
	   ...
	   * @param {object} chart instance
	   */
	  var setTurnoversTooltipPosition = function setTurnoversTooltipPosition(element, tooltip, chart) {
	    var dataPoint = tooltip.dataPoints[0];
	    var positionX = chart.canvas.offsetLeft + Math.floor(dataPoint.x) - Math.ceil(element.clientWidth / 2);
	  ...
	  return {
	    ...
	    customizeTooltip: function customizeTooltip(tooltip, element, data, chart) {
	      ...
	      var parent = document.querySelector(_constants.DEFAULT_WIDGET_SELECTOR);
	      ...
	      var isNarrow = chart.width + element.offsetWidth * 2 + 12 * 2 * 2 + _constants.CHART_SECTOR_SHIFT_SIZE * 2 > parent.offsetWidth;
	      ...
	      if (isNarrow) {
	        arrowClass = item.totalPortion - item.portion / 2 < 50 ? 'bb-arrow-right' : 'bb-arrow-left';
	      } else {
	        arrowClass = item.totalPortion - item.portion / 2 > 50 ? 'bb-arrow-right' : 'bb-arrow-left';
	      }
	     ...
	     */
	    customizeTurnoversTooltip: function customizeTurnoversTooltip(tooltip, element, data, chart) {
	      // get all data for active point
	      var item = data.original.turnovers[tooltip.dataPoints[0].index];
	      var dataKey = data.datasets[0].dataKey;
	
	      Object.assign(element, {
	        innerHTML: '\n          <div class="row">\n            <span class="col-5">\n              ' + (isSpending ? i18nFilter('analysis.tooltip.title.spent') : i18nFilter('analysis.tooltip.title.income')) + '\n            </span>\n            <span class="col-7 pl-0 text-right">\n              <ui-bb-format-amount-ng class="amount-regular-color"\n                data-amount="' + item[dataKey].amount + '"\n                data-currency="\'' + item[dataKey].currencyCode + '\'"\n                data-wrap>\n              </ui-bb-format-amount-ng>\n            </span>\n          </div>\n        ',
	        className: 'chart-tooltip chart-tooltip-default chart-tooltip-small text-center\n            bb-arrow-bottom'
	      });
	    ...
	    onPeriodEndChange: function onPeriodEndChange(ctrl) {
	      return function (newEndDate) {
	        ...
	        ctrl.updateAnalysisCategories().then(ctrl.setSelectedAnalysisItem(null));
	      };
	    },
