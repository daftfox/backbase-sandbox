...
SET @sys2sys_sid = LAST_INSERT_ID();
insert into acl_sid (principal, sid) values (false, 'group_sys2sys');
SET @group_sys2sys_sid = LAST_INSERT_ID();


INSERT INTO items (discriminator,state,name,parentItemName,template_id,
contextItemName,subType,type,userId,extendedItem_id,createdTimestamp,lastModifiedTimestamp, uuid, publicationStatus)
VALUES ('PortalFoundation','INSTANTIATED','[BBHOST]',null,null,
'[BBHOST]',null,'PORTALFOUNDATION',null,null,now(), now(),
'413c44e2-59b8-47b3-997a-427f224b14a7', 'NOT_PUBLISHED');
...
-- INSERT INTO acl_object_identity (object_id_class, object_id_identity, owner_sid, entries_inheriting) VALUES (8, @item_id, @group_sys2sys_sid, false);
-- SET @aoi_sys2sys_id = LAST_INSERT_ID();
