...
values
    ((select id from acl_class where class='com.backbase.portal.foundation.domain.model.PortalFoundation'),
     (select ID from items where discriminator='PortalFoundation'),
     (select id from acl_sid where sid='group_admin'), 0)
GO
